...
class ConsoleProcess(object):
    def __init__(self, roms_path, game_id, render=True, throttle=False, frame_skip=0, sound=False, mame_bin_path='mame', other_args = '', ip='127.0.0.1', port=12000):
        ...
        command = f'{mame_bin_path} {game_id} -rompath "{os.path.abspath(roms_path)}" -skip_gameinfo -window -nomaximize -noverbose'
        command += f' -autoboot_script {lua_path}/kof98.lua'
        if not render:
            command += " -video none"
